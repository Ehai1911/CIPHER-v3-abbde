...
async function callAI(provider, apiKey, systemPrompt, userPrompt, maxTokens) {
  if (provider === 'anthropic') {
    const resp = await httpPost('api.anthropic.com', '/v1/messages',
      ...
      { model: 'claude-3-5-sonnet-20241022', max_tokens: maxTokens, system: systemPrompt, messages: [{ role: 'user', content: userPrompt }] }
    );
...
const COMPLEX_TABS = new Set(['strategy', 'swot', 'quickwins', 'gaps', 'intel']);
...
const TAB_SCHEMAS = {
  ...
  intel: `{"intel":{"direct":[{"name":"Компания1","url":"https://competitor1.com","description":"Одно предложение что делает и кому","pricing":"от $X/мес","audience":"кто покупает","position":"Лидер","ux":"Сильная сторона UX"},{"name":"Компания2","url":"https://competitor2.com","description":"Одно предложение","pricing":"от $X/мес","audience":"аудитория","position":"Претендент","ux":"сильная сторона"},{"name":"Компания3","url":"https://competitor3.com","description":"Одно предложение","pricing":"от $X/мес","audience":"аудитория","position":"Нишевой","ux":"сильная сторона"},{"name":"Компания4","url":"https://competitor4.com","description":"Одно предложение","pricing":"от $X/мес","audience":"аудитория","position":"Нишевой","ux":"сильная сторона"},{"name":"Компания5","url":"https://competitor5.com","description":"Одно предложение","pricing":"от $X/мес","audience":"аудитория","position":"Растущий","ux":"сильная сторона"}],"indirect":[{"name":"Компания","description":"что делает","why":"почему решает ту же задачу пользователя","type":"тип продукта"},{"name":"Компания2","description":"что делает","why":"релевантность","type":"тип продукта"},{"name":"Компания3","description":"что делает","why":"релевантность","type":"тип продукта"}],"benchmarks":[{"name":"Linear","industry":"Project Management","why":"Лучший UX онбординга — 3 мин до первой задачи"},{"name":"Stripe","industry":"Fintech","why":"Документация и developer experience как эталон простоты"}],"recommended":["Компания1","Компания2","Компания3"],"recommendation_why":"Объяснение почему именно эти 3 для глубокого анализа — чем они важны для ${product}"}}`
};
...
Поле "productIdeas" — ровно 3 конкретных идеи продуктов или функций которых НЕТ ни у одного конкурента, основанных на реальных пробелах рынка из анализа. Поле "name" — конкретное название. Поле "problem" — кому нужно и какую боль закрывает. Поле "missingAt" — перечисли 2-3 конкурентов у которых этого нет. Поле "demand" — только 🔥 Высокий / ⚡ Средний. Поле "effort" — только Низкая / Средняя / Высокая.` : '';
  ...
  const tabInstructions = {
    ...
    intel: `Составь полную карту конкурентного ландшафта для ${product}.
DIRECT (5-8 прямых конкурентов): та же категория, та же аудитория. description — одно конкретное предложение что делает. pricing — реальная цена. position — только: Лидер / Претендент / Растущий / Нишевой. ux — одна заметная сильная сторона UX (онбординг, скорость, простота, дизайн и т.д.).
INDIRECT (3-5 непрямых): другой тип продукта но решает ту же задачу пользователя что и ${product}. why — конкретно почему пользователь может выбрать их вместо нас.
BENCHMARKS (ровно 2): компании из СМЕЖНЫХ индустрий (не конкуренты) которые решают похожую задачу взаимодействия на высшем уровне. why — конкретная причина почему они эталон для нашего UX/продукта.
RECOMMENDED (3-5 из direct): какие конкуренты важнее всего изучить глубоко и почему — привяжи к конкретным слабостям ${product}.`
  };
