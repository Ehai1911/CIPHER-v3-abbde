...
Поле "productIdeas" — ровно 3 конкретных идеи продуктов или функций которых НЕТ ни у одного конкурента, основанных на реальных пробелах рынка из анализа. Поле "name" — конкретное название. Поле "problem" — кому нужно и какую боль закрывает. Поле "missingAt" — перечисли 2-3 конкурентов у которых этого нет. Поле "demand" — только 🔥 Высокий / ⚡ Средний. Поле "effort" — только Низкая / Средняя / Высокая.` : '';
  ...
  const prompt = `Ты — ведущий аналитик конкурентной разведки. Продукт: ${product} | Сфера: ${area} | Сегмент: ${segment} | Цена: ${price}
Описание: ${description}

ЦЕЛЕВЫЕ РЫНКИ (ищи конкурентов именно на этих рынках, не ограничивайся одной страной):
${geo}
...
${tabHint}
ПРАВИЛА:
- Замени Н1–Н5 на реальные названия компаний-конкурентов в сфере "${area}" с указанных рынков. НашПродукт → "${product}".
- Включай конкурентов из ВСЕХ указанных рынков, не только из одного.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const GEO_CONTEXT = {
      'казахстан': 'Казахстан: локальные игроки (Kaspi, Kolesa, Chocofood, Jusan), ценообразование в тенге, B2B-специфика Алматы и Астаны',
      'снг': 'СНГ (все страны): Россия, Казахстан, Беларусь, Украина, Узбекистан, Азербайджан, Армения, Молдова, Таджикистан, Кыргызстан — русскоязычный рынок, учитывай локальных игроков каждой страны',
      'европа': 'Европа: Западная (Германия, Великобритания, Франция, Нидерланды, Испания, Италия, Швеция), Восточная (Польша, Чехия, Румыния, Венгрия), Балтия (Латвия, Литва, Эстония) — учитывай GDPR и евро-ценообразование',
      'сша': 'США и Канада: крупнейший SaaS-рынок, учитывай венчурные стартапы, Product Hunt-компании, ценообразование в долларах',
      'ближний восток': 'Ближний Восток: ОАЭ, Саудовская Аравия, Катар, Кувейт, Бахрейн, Оман — учитывай Vision 2030, стартап-экосистему DIFC Дубая, арабскую локализацию',
      'оаэ': 'ОАЭ: стартап-хаб Дубай (DIFC), экосистема Абу-Даби, мультикультурная аудитория, английский и арабский языки, налог на прибыль 9%',
      'индия': 'Индия: локальные гиганты (Zoho, Freshworks, Tata Consultancy, InMobi), ценообразование в рупиях, tier-1 города (Бангалор, Мумбаи, Дели), огромный SMB-рынок',
      'китай': 'Китай: закрытый рынок с собственной экосистемой — Alibaba, Tencent, Baidu, ByteDance, Meituan. Google/Facebook не работают, основной канал — WeChat, ценообразование в юанях, обязательны локальные партнёры',
      'весь мир': 'Глобальный рынок: США, Европа, Индия, Китай, Ближний Восток, Латинская Америка, Юго-Восточная Азия, СНГ — анализируй международных лидеров и региональных игроков',
      'латинская америка': 'Латинская Америка: Бразилия, Мексика, Колумбия, Аргентина, Чили — ценообразование в реалах/песо, локальные игроки (Mercado Libre, Rappi, Nubank)',
      'юго-восточная азия': 'Юго-Восточная Азия: Сингапур, Индонезия, Вьетнам, Таиланд, Малайзия, Филиппины — быстрорастущие рынки, Grab, Gojek, Sea Group как локальные гиганты',
      'африка': 'Африка: ЮАР, Нигерия, Кения, Марокко — мобильные платежи (M-Pesa), быстрый рост финтеха, молодая аудитория',
    };

    const rawGeo = Array.isArray(geography) ? geography.join(', ') : (geography || 'глобально');
    const geoLower = rawGeo.toLowerCase();
    const geoExpanded = Object.entries(GEO_CONTEXT)
      .filter(([key]) => geoLower.includes(key))
      .map(([, val]) => val)
      .join('; ');
    const geo = geoExpanded || rawGeo;
